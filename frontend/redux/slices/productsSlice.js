...
export const fetchProductsByFilters = createAsyncThunk("products/fetchByFilters", async ({
    ...
    const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/products?${query.toString()}`);
...
const productSlice = createSlice({
    ...
    initialState: {
        products: [],
        selectedProduct: null, // Store the details os the single product
        similarProducts: [],
        loading: false,
        error: null,
        filters: {
    ...
    extraReducers: (builder) => {
        builder
            // handling fetching products with filter
